...
- 用户 id 的数量dmin=50：即参与免费试学的用户数量
  - 虽然该指标的统计发生在变更之后，会受到影响，但是因为实验组和对照组的 cookie 数量不一定相同，该指标的变化也许不是受变更影响，而是因为 cookie 数不同导致。因此该指标不能很好地评估变更效果，而比例化的评估量总转化率更适合作为评估指标
